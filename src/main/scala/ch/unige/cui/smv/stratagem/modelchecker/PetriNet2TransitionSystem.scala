...
object PetriNet2TransitionSystem {
  ...
  private def createSignature(places:List[Place]) : Signature =  places match{
    case Nil => basicSignature
    case place::tail => createSignature(tail).withGenerator(place.id, "place", "nat", "place")
  }
  
  private def createTransitionSystem(transitions:List[Transition], adt: ADT, initialState: ATerm) : TransitionSystem =  transitions match {
  ...
  private def createInitialState(places:List[Place], adt:ADT):ATerm = places match {
  ...
  private def createStrategyFor(transition: Transition, adt:ADT, ts: TransitionSystem):TransitionSystem = {
  ...
  private def createOutputArcEquations(arcs:List[Arc], adt:ADT, ts:TransitionSystem):TransitionSystem = arcs match {
    case Nil => ts
  ...
  private def createInputArcEquations(arcs:List[Arc], adt:ADT, ts:TransitionSystem):TransitionSystem = arcs match {
    case Nil => ts
    case arc :: tail => 
