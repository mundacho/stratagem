 ...
 *
 */
object SetOfModules2TSWithAnonimizationAndPOR extends Logging with ((List[PTModule], PetriNet) => TransitionSystem) {
    ...
    transitions match {
      case Nil => // for each localCluster we create a fixpoint strategy to be applied on that cluster
        ...
        localClusterStrat.keys.view.toList.sortWith(_ < _).foreach { i =>
          ...
          localClusterStrat.keys.view.toList.sortWith(_ < _).takeWhile(_ < i).reverse.headOption match {
            case None => resTransSystem = resTransSystem.declareStrategy(s"doFor${i}thCluster") { ApplyToCluster(strategyBody, i) }(true)
            case Some(j) => logger.trace(s"Declared doFor${i}thCluster as a function of doFor${j}thCluster")
              resTransSystem = resTransSystem.declareStrategy(s"doFor${i}thCluster") { Sequence(DeclaredStrategyInstance(s"doFor${j}thCluster"), ApplyToCluster(strategyBody, i)) }(true)
          }
  ...
  private def buildSequenceOfDependentStrategies(l: List[NonVariableStrategy]): NonVariableStrategy = l.reduceRight(ApplyOnceAndThen(_, _))
