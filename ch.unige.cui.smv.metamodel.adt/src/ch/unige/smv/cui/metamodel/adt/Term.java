 ...
 *        annotation="http://www.eclipse.org/emf/2002/Ecore/OCL/Pivot CorrectNumberOfParameters='subterms->size() = operationSymbol.formalParameters->size()' CorrectNumberOfParameters$message='\'Invalid number of parameters for term: \' + self.toString() + \'. Required \' + operationSymbol.formalParameters->size()->toString() + \', found \' + subterms->size()->toString()' CorrectTypeOfParameters='subterms\n\t\t\t->forAll(p | p.sort.isSubSortOf(operationSymbol.formalParameters\n\t\t\t\t\t->at(subterms\n\t\t\t\t\t\t->indexOf(p))))' CorrectTypeOfParameters$message='\'Invalid type of parameters for term: \' + self.toString()' RightAdtSubterms='subterms->forAll(t | t.adt = self.adt)' RightAdtSubterms$message='\'Some subterms do not have the same adt as the parent term: \' + self.toString()'"
 * @generated
 */
